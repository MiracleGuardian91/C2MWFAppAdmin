...
(function($) {
    $.fn.droptabs = function(o) {
        ...
        return this.each( function(  ) {
			...
			function manageActive(elem) {
				...
				$('a', $(elem)).on('shown.bs.tab', function (e) {
					if ($(dropdown).hasClass('active')) {
						$('>a', dropdown).html(($('>li.active>a', dropdownMenu).html()).substring(0,10) + '... <i class="fas fa-chevron-down"></i>');
					} else {
						$('>a', dropdown).html('See More <i class="fas fa-chevron-down"></i>'); 
					}
				})
			...
			if ( s.development ) {		
				//$('body').append('<div class="alert alert-success" id="'+ s.developmentId +'" style="display:none!important;"></div>');
				var $developmentDiv = $('#' + s.developmentId);
				$($developmentDiv).css('position','fixed').css('right','20px').css('display','none').css('bottom','20px');				
