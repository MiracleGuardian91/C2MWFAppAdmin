...
import { EContractService } from '@app/core/services/e-contract.service';
...
import { GridViewConfigurationComponent } from '../grid-view-configuration';
      ...
      if (!this.IsPermissionSet) {
        ...
        this.listviewService.userActionPermission(this.ProcessName).subscribe(
          data => {
            ...
            this.IsBulkTriggerAllow = data.IsBulkTriggerAllow;
...
//Entity related code– Nidhi 
  ...
  async openBulkUpload() {
    const bu = await import('@lib/feat/bulk-upload').then(c => c.BulkUploadComponent);
    const modal = this.msg.showComponent(bu, {
    ...
    if (response.status === 'Success') {
      const {TotalCount, PassCount, ErrorCount} = response.result;
      const message = "Total Count: " + TotalCount + ", " + "Pass Count: " + PassCount + " and " + "Error Count: " + ErrorCount;
      this.getGridConfigData();
      ...
      this.msg.showMessage('Fail', { body: response.Message });
    }
  }
