...
export class ApiService {
 ...
 async postAdvanceSearch(url: string, data: any,  parameters?: any){    
  ...
  postGateway(url: string, data: any, parameters?: any): Observable<any> {
    const params = this.setParams(parameters);
  ...
  postgateway1(endPoint: string, url: string, data: any, parameters?: any): Observable<any> {
  const headers = this.setHeaders(data);
  const params = this.setParams(parameters);
  ...
  const finalUrl =
    endPoint && endPoint !== ""
      ? `${endPoint}/${url}`
      : `${this.GatewayAPIUrl}/${url}`;

  return this.withLoader(
    finalUrl,
    this.http.post<any>(finalUrl, data, { headers, params })
  );
}

  postgateway(endPoint: string, url: string, data: any, parameters?: any): Observable<any> {
    const fullUrl = endPoint != null && endPoint != "" 
      ? `${endPoint}/${url}` 
      : `${this.GatewayAPIUrl}/${url}`;
    ...
    const request$ = this.http.post<any>(fullUrl, data, { headers, params });
  ...
  SaveBMInsertCardGroup(bmoId: string): Observable<any> {
